...
def hao_orlin_directed(G, s) -> list[Partition]:
    ...
    def discharge(u):
        ...
        Discharges the excess flow from node u.
        """
        # Zuerst versuchen wir, einen Push durchzuführen
        for v in G.neighbors(u):
            # Überprüfe, ob ein Push möglich ist: Restkapazität vorhanden und Höhenbedingung erfüllt
            if G.nodes[u]['height'] == G.nodes[v]['height'] + 1 and G.edges[u, v]['capacity'] - G.edges[u, v]['preflow'] > 0 and v not in X and G.nodes[u]['excess'] > 0:
                push(u, v)
                return  # Push durchgeführt, wir verlassen die Funktion
    ...
    while X != V:
        ...
        yeh_list.append(Partition(min_cut_value, {'P': P, 'cut': cut, 'residual_graph': original_graph}))
