...
def minimum_s_cut(G: nx.DiGraph, child_vector) -> tuple[int | float, tuple[set, set]]:
    min_cut = (math.inf, ())
    for x in range(0,(len(G.nodes)-len(child_vector))): 
...
def global_min_cut_undirected(G: nx.DiGraph) -> tuple[int | float, tuple[set, set]]:
    ...
    for node in nodes[1:-1]: 
        ...
        min_cut1=partly_specified_min_cut(G, v1) #child_min_value, child_min_partition = partly_specified_min_cut(G, child_vector)
        min_cut2=partly_specified_min_cut(G, v2)
    ...
    return (min_cut[0],get_original_partition(G, min_cut[1], vector))
